...
public class Function
{
    ...
    {
        Func<Stream, ILambdaContext, string> handler = FunctionHandler;
        await LambdaBootstrapBuilder.Create(handler, new SourceGeneratorLambdaJsonSerializer<LambdaFunctionJsonSerializerContext>())
        //await LambdaBootstrapBuilder.Create(handler, new SourceGeneratorLambdaJsonSerializer<HttpApiJsonSerializerContext>(option =>
        //{
    ...
    /// <returns></returns>
    public static string FunctionHandler(Stream input, ILambdaContext context)
    {
        ...
        var request = JsonSerializer.Deserialize<ProductRequest>(jsonString);
        var result = new ProductResponse()
        {
            ResultCode = "000",
            ResultMsg = "Success" //不支援中文
        };
        return JsonSerializer.Serialize<ProductResponse>(result);
    }
}
